...
import yoonsome.mulang.api.student.service.QnaService;
...
public class QnaController {
    ...
    public String qna(Model model, @AuthenticationPrincipal CustomUserDetails userDetails, @RequestParam(defaultValue = "최신순") String sort) {
        ...
        List<QnaResponse> myqna;
        if ("오래된순".equals(sort)) {
            myqna = qnaService.getQuestionPageByUserAsc(userId);   //오래된순
        } else {
            myqna = qnaService.getQuestionPageByUserDesc(userId);  // 최신순 (기본)
        }
